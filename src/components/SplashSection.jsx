  ...
  useEffect(() => {
    ...
    class Globe {
      ...
      initializeGlobe(canvasWidth, canvasHeight) {
        ...
        this.globeRadius =
          canvasWidth < canvasHeight ? canvasWidth / 2 : canvasHeight / 2;
    ...
    class Particle {
      constructor(effect, x, y, color) {
        ...
        this.distance = 0;
        this.friction = Math.random() * 0.6 + 0.1; //Math.random() * 0.6 + 0.15;
        this.ease = 0.045;
      ...
      spawnGlobe() {
        ...
        if (
          this.globeStartX + 2 >= this.currX &&
          this.globeStartY + 2 >= this.currY
        ) {
      ...
      refreshGlobeParticle() {
        ...
        const rotationX =
          cos * this.globeX + sin * (this.globeZ - globe.globeCenterZ);
        const rotationZ =
          -sin * this.globeX +
          cos * (this.globeZ - globe.globeCenterZ) +
          globe.globeCenterZ;

        const orbitSizeProjection =
          globe.globePerspective / (globe.globePerspective - rotationZ);
        const globeX =
          rotationX * orbitSizeProjection +
          globe.globeCenterX -
          heroCanvas.globeParticleSize;
        const globeY =
          this.globeY * orbitSizeProjection +
          globe.globeCenterY -
          heroCanvas.globeParticleSize;
      ...
      formText() {
        ...
        this.currSize =
          this.particleSizeFactor *
            (1 -
              (this.particleStartDist - currentDistance) /
                this.particleStartDist) +
          this.finalSize;
      ...
      interactPointer() {
        ...
        this.currX +=
          (this.vx *= this.friction) + (this.finalX - this.currX) * this.ease;
        this.currY +=
          (this.vy *= this.friction) + (this.finalY - this.currY) * this.ease;
      }
    }
    ...
    class Effect {
      constructor() {
        ...
        heroCanvas.canvas.addEventListener("touchstart", handleTouchStart, {
          passive: false,
        });
      ...
      async setupText(isSkipGetFont) {
        ...
        const gradient = heroCanvas.ctx.createRadialGradient(
          heroCanvas.width / 2,
          heroCanvas.height / 2,
          0,
          heroCanvas.width / 2,
        ...
        linesArray.forEach((el, index) => {
          heroCanvas.ctx.fillText(
            el,
            this.textX,
            this.textY + index * heroCanvas.lineHeight
          );
          heroCanvas.ctx.strokeText(
            el,
            this.textX,
            this.textY + index * heroCanvas.lineHeight
          );
      ...
      convertToParticles() {
        this.particles = [];
        const pixels = heroCanvas.ctx.getImageData(
          0,
          0,
          heroCanvas.width,
          heroCanvas.height
        ...
        for (
          let y = 0;
          y < heroCanvas.height;
          y += heroCanvas.letterParticleSize
        ) {
          for (
            let x = 0;
            x < heroCanvas.width;
            x += heroCanvas.letterParticleSize
          ) {
    ...
    return () => {
      ...
      heroCanvas.canvas.removeEventListener("touchstart", handleTouchStart, {
        passive: false,
      });
  ...
  return (
    <div className="relative w-[100%] flex flex-col flex-nowrap items-center justify-center gap-0 sm:gap-8 md:gap-9 lg:gap-10">
      <div className="relative sm:absolute sm:top-0 sm:right-0 flex flex-col flex-nowrap items-center justify-center mx-mobileBound sm:mr-8 text-center">
        ...
        <svg
          id="canvas-arrow"
          className="opacity-0 transition-[opacity] duration-[1000ms] absolute sm:relative top-8 sm:top-auto animate-hero-pointer-bounce-mobile sm:animate-hero-pointer-bounce sm:mt-0 h-9 sm:h-10 md:h-11 lg:h-[80px]"
          enableBackground="new 0 0 122.433 122.88"
          version="1.1"
          viewBox="0 0 52.513 52.695"
          xmlns="http://www.w3.org/2000/svg"
        >
          <title>Arrow pointing to animation canvas</title>
          <g
            transform="matrix(.4069 0 0 .4069 1.3475 1.3475)"
            fill="hsl(var(--color-primBlue-200))"
            style={{ mixBlendMode: "normal" }}
          >
            <polygon
              points="0 59.207 39.403 59.207 39.403 0 83.033 0 83.033 59.207 122.43 59.207 61.216 122.88"
              clipRule="evenodd"
              fill="hsl(var(--color-primBlue-200))"
              fillRule="evenodd"
            />
          ...
          >
            <polygon
              points="39.403 59.207 39.403 0 83.033 0 83.033 59.207 122.43 59.207 61.216 122.88 0 59.207"
              clipRule="evenodd"
              fill="hsl(var(--color-primBlue-200))"
              fillRule="evenodd"
            />
