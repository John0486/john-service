...
import { MenuIcon, CloseIcon, SunIcon, MoonIcon, ShareIcon } from "./Icons";
      ...
      <>
        <ul className="flex flex-row flex-nowrap justify-between items-center sm:order-1">
          <li>
            <button
              ...
              aria-label={
                isDarkMode
                  ? "Toggle site to light mode"
                  : "Toggle site to dark mode"
              }
        ...
        <nav>
          <ul className="flex flex-col flex-nowrap items-center gap-7 my-7 sm:flex-row sm:gap-6 md:gap-9 lg:gap-11">
            {navMenu.map((menuItem) => (
              <li
                key={menuItem.id}
                className="flex flex-col flex-nowrap justify-center items-center w-[100%]"
              >
                <NavLink
                  to={menuItem.href}
                  className={({ isActive, isPending }) =>
                    `${
                      isActive
                        ? "text-primBlue-900 bg-primBlue-200 px-5 font-bold"
                        : isPending
                        ? "text-neutGray-800"
                        : ""
                    } w-[80%] sm:w-[100%] text-center py-4`
                  }
        ...
        <div className="flex flex-col flex-nowrap justify-center items-center sm:fixed sm:left-[8px] sm:bottom-[14px] sm:w-[50px] ">
          <div
            ...
            className={`collapsible-container max-h-auto sm:relative sm:max-h-0 sm:overflow-hidden sm:transition-[max-height] sm:duration-[250ms] sm:ease-out ${
              isSocialOpen ? "sm:max-h-[500px]" : ""
            }`}
            aria-hidden={
              refIsMobileWidth.current
                ? "false"
                : isSocialOpen
                ? "false"
                : "true"
          ...
          <button
            ...
            aria-label={
              isSocialOpen
                ? "Close social media panel"
                : "Open social media panel"
            }
