...
export default function ProjectModal({ projectId, projectData }) {
  ...
  useEffect(() => {
    const dialog = document.getElementById(
      `closeModal-${projectModalId.current}`
    );
  ...
  return (
    <>
      <button aria-label={projectData.ariaLabel} onClick={toggleDialog}>
        <span className="inline-link max-w-[300px] font-heading font-bold text-6 sm:text-5 md:text-4 lg:text-3 max-w-read text-center mx-auto;">
          {projectData.title}
        </span>
      </button>
        ...
        <div className="w-[100%] p-9 flex flex-col flex-nowrap gap-9">
          ...
          {projectData.details.description !== null && (
            <>
              ...
              <p className="ml-auto mr-auto text-justify">
          ...
          <ul className="w-[100%] grid grid-cols-[repeat(auto-fit,minmax(80px,1fr))] md:grid-cols-[repeat(auto-fit,minmax(90px,1fr))] lg:grid-cols-[repeat(auto-fit,minmax(110px,1fr))] justify-items-center items-start justify-center gap-6">
            {projectData.details.technologies.map((technology, id) => (
              <li
                key={id}
                className="h-[100%] flex flex-col flex-nowrap items-center gap-5 justify-center"
              >
                <technology.Icon />
                <span className="text-center hyphens-auto">
                  {technology.title}
                </span>
              </li>
            ))}
          ...
          <div className="flex flex-row flex-wrap gap-7 items-center justify-center">
            {projectData.details.url !== null && (
              <a
                href={projectData.details.url}
                className="glassy-icon px-6 shrink-0"
              >
                Visit Website
              </a>
            )}
            {projectData.details.githubUrl !== null && (
              <a
                href={projectData.details.githubUrl}
                className="glassy-icon px-6 shrink-0"
              >
